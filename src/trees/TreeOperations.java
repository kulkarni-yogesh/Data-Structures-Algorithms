...
class TreeOperations {
	...
	public static Node insert2(Node head, Node parent, int val, boolean isLeft) {
		Node root = head;
		if (root == null) {
			root = new Node(val);
			if (parent != null) {
	...
	public boolean deleteNode(int val) {
		...
		// Case 3 both children
			...
			// Case 3b: Successor is not the right child of node to be deleted but in the
			// right sub-tree
			else {
...
//------------------- Alternative Code to DElete a Node in Tree
	 ...
	 * @param root : root of the tree
	 * @return : -1 if not found or tree is null else return the value of the Node
	...
	public static void zigZag1(Node root, int currLevel, int level, boolean reverseOrder) {
		if (root == null)
			return;
	...
	public static boolean hasPathSum(Node root, int sum, String path) {
		...
		return hasPathSum(root.left, sum - root.data, path + "-->" + root.data)
				|| hasPathSum(root.right, sum - root.data, path + "-->" + root.data);
	}
	...
	public static void verticalTraversal(Node root, int col, Map<Integer, ArrayList<Node>> hash) {
		if (root == null)
			return;
